...
parser = argparse.ArgumentParser(
    description="Second step: Generate summary table of hmmsearch results for ViPhOGs vs UniProt, for each chunk file. Make sure to run the script from within the directory containing the domtbl output directories (check comment block for guidance) and following the script that executes Step 1"
)
parser.add_argument(
    "-i", "--input", dest="input_file", help="Path to input chunk file", required=True
)
            ...
            for line in input_file:
                ...
                for elem in tax_ranks_list:
                    ...
                    for key, value in taxid_dict.items():
                        try:
                            ...
                            for x, y in lineage_ranks.items():
                                if y == elem and lineage_names[x] not in elem_results:
                    ...
                    results_per_rank.append(
                        [
                            (taxon, features[0], max(features[1]), min(features[1]))
                            for taxon, features in elem_results.items()
                        ]
                    )
                output_file.write(
                    f"{viphog_id}\t{unique_hits}\t{results_per_rank[0]}\t{results_per_rank[1]}\t{results_per_rank[2]}\t{results_per_rank[3]}\n"
                )
