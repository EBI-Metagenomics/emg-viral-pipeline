...
print(
    "First step: Sort ViPhOGs into chunks based on the number of different hits against UniProtKB\n"
)
print("Looking for domtbl output files...")
if len([item for item in glob.glob("hmm*domtbl") if os.path.isdir(item)]) < 1:
    print(
        "No domtbl output directories found.\nMake sure to run the script from within the directory containing the domtbl output directories (check comment block for guidance)",
        file=sys.stderr,
    )
else:
