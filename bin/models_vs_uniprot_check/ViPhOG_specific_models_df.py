...
parser = argparse.ArgumentParser(
    description="Step 4: Generate pickled dataframe of ViPhOGs selected as viral taxonomic markers. Make sure to run the script from within the directory containing the domtbl output directories (check comment block for guidance) and following the scripts that execute Step 1, Step 2 and Step 3",
    add_help=False,
)
parser.add_argument(
    "-h",
    ...
    dest="help",
    action="store_true",
    help="Use flag to print this help message",
)
...
if args.help:
    ...
    if not os.path.exists("unique_hits_chunks"):
                ...
                for elem in summary_files:
                    with open(elem) as input_file:
                        ...
                        for row, data in input_df.iterrows():
                            if (
                                data["#_taxons"] != 0
                                and pd.isnull(data["Overlapping_taxons"])
                                and data["ViPhOG"] not in overall_viphog_list
                            ):
                                overall_viphog_list.append(data["ViPhOG"])
                                viphog_list.append(data["ViPhOG"])
                                hit_number_list.append(data["#_taxons"])
                                min_score_list.append(
                                    ast.literal_eval(data["Max_min_score"])[1]
                                )
                                ...
                                taxon_lineage = ncbi.get_lineage(
                                    taxon_name_id[taxon_name][0]
                                )
                                taxon_lineage_names = ncbi.get_taxid_translator(
                                    taxon_lineage
                                )
        ...
        print("Done!")
        print(
            "Adding information about host type for viral taxa linked to specific ViPhOGs..."
        )
        if not os.path.exists("virushostdb.tsv"):
            print(
                "virushostdb.tsv file not found, can not add host type info to dataframe"
            )
        else:
            ...
            for row, data in virus_to_host_df.iterrows():
                if pd.notnull(data["host lineage"]) and pd.notnull(
                    data["virus lineage"]
                ):
                    if data["host lineage"].split("; ")[0] == "Eukaryota":
                        ...
                        data["host lineage"].split("; ")[0] == "Bacteria"
                        or data["host lineage"].split("; ")[0] == "Archaea"
                    ):
                        for item in data["virus lineage"].split("; "):
                            org_type_dict["Prokaryote"].add(item)
            org_type_dict["Eukaryote"].difference_update(
                {
                    "Caudovirales",
                    "Microviridae",
                    "Podoviridae",
            ...
            org_type_dict["Eukaryote"].update(
                {"Catovirus", "Klosneuvirus", "Klosneuvirinae"}
            )
