...
parser = argparse.ArgumentParser(
    description="Step 3: Generate summary tables for each taxonomic rank. Make sure to run the script from within the directory containing the domtbl output directories (check comment block for guidance) and following the scripts that execute Step 1 and Step 2"
)
parser.add_argument(
    "-i", "--input", dest="input_file", help="Path to summary chunk file", required=True
)
    ...
    with open(summ_file) as input_file:
        ...
        for x, y in taxa_ranks:
            ...
            with open(f"{os.path.splitext(summ_file)[0]}_{y}.tsv", "w") as output_file:
                output_file.write(
                    "ViPhOG\t#_taxons\tMost_significant\tMax_min_score\tOverlapping_taxons\tNext_max_score\n"
                )
                for line in input_file:
                    ...
                    next_max_score = ""
                    if total_hits > 0:
                        rank_hits_sorted = sorted(
                            rank_hits, key=operator.itemgetter(2), reverse=True
                        )
                    ...
                    output_file.write(
                        f"{viphog_id}\t{total_hits}\t{most_significant}\t{score_range}\t{overlap}\t{next_max_score}\n"
                    )
