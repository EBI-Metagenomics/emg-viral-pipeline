...
def parse_virus_sorter2(sorter_files):
    ...
    for record in SeqIO.parse(final_combined_fa_file, "fasta"):
        ...
        max_score = meta.loc[clean_name, 'max_score']
        circular = meta.loc[clean_name, 'shape'].startswith('c')
        prange = [meta.loc[clean_name, 'trim_bp_start'], meta.loc[clean_name, 'trim_bp_end']]
