            ...
            for rank in tax_rank_order:
                ...
                if total_hits == 0:
                    ...
                    for hit_taxon, hit_count in count_hits.items():
                        ...
                        taxon_factor = taxon_factor_dict[hit_taxon]['factor'] if hit_taxon in taxon_factor_dict.keys() else 1
                    ...
                    if len(over_thres) == 0:
                        #best_under = sorted(under_thres, key=lambda x: x[1])[0][1]
                        best_under = sorted(under_thres, key=lambda x: x[1])[0]
                        contig_lineage.append(best_under[1])
                    else:
                        sorted_over_thres = [x for x,y in sorted(over_thres, key=lambda x: x[1], reverse=True)]
                        for taxon in sorted_over_thres:
                            if taxon in taxon_factor_dict.keys() and total_prot <= taxon_factor_dict[taxon].get('avg_cds') + 2*taxon_factor_dict[taxon].get('std_cds'):
                                taxon_taxid = ncbi.get_name_translator([taxon])[taxon][0]
                                taxon_lineage = ncbi.get_lineage(taxon_taxid)
                                taxon_lineage_dict = {y:ncbi.get_taxid_translator([x])[x] for x,y in ncbi.get_rank(taxon_lineage).items() if y in tax_rank_order[tax_rank_order.index(rank):]}
                                taxon_lineage_list = [taxon_lineage_dict.get(item, "") for item in tax_rank_order[tax_rank_order.index(rank):]]
                                break
                        else:
                            contig_lineage.append("")
                            continue
