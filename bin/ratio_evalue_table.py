...
def ratio_evalue(vphmm_df, taxa_dict, evalue):
    ...
    for vphmm in vphmm_hits:
        vphmm_specific_df = informative_df[
            informative_df["target name"] == vphmm
                ...
                coords_list = sorted(
                    [
                        (
                            ...
                            vphmm_specific_df["hmm coord to"][i + j],
                        )
        ...
        final_df = pd.DataFrame(
            final_hit_list, columns=["ViPhOG", "query", "Ratio", "Abs_Evalue_exp"]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--taxa",
        dest="taxa_tsv",
        help="TSV file: additional_data_vpHMMs_v{1,2,3,4}.tsv",
        required=True,
    )
    parser.add_argument(
        "-o",
    ...
    for i in range(len(tsv_df)):
        taxa_dict["ViPhOG" + str(tsv_df["Number"][i]) + ".faa"] = tsv_df["Associated"][
            i
        ]
    ...
    with open(output_file, "w") as of_handle:
        if output_df is None or output_df.empty:
            print(
                "No informative hits against the ViPhOG database "
